...
static void *handle_conn(void *arg)
{
    ...
    size_t csize = ZSTD_compress(cbuf, bound, body.data, body.len, ZSTD_LEVEL);
    ...
    fprintf(stderr,
            "[z-egress] %s %s  %zu -> %zu B  (%.1f%% saved, %.0f us)\n",
            req.method, req.target, body.len, csize,
            100.0 * (1.0 - (double)csize / (double)body.len), us);
